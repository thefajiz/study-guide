...
function Get-InstalledDistros {
    ...
    $distros = foreach ($line in $output) {
        if ($null -eq $line) {
            continue
        }

        # WSL may return output containing hidden/null characters.
        # Strip them before matching distro names.
        $name = $line.ToString().Replace([string][char]0, '').Trim()

        if ($name -and $name -notmatch '(?i)^Windows Subsystem') {
            $name
        }
    }
